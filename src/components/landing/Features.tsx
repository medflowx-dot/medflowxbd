...
const Features = () => {
  const { data: cmsContent } = useCMSContent('features');
  
  const title = getCMSValue(cmsContent, 'title', 'সম্পূর্ণ মেয়াদ ও আর্থিক ট্র্যাকিং');
  const subtitle = getCMSValue(cmsContent, 'subtitle', 'বাংলাদেশি ফার্মেসির জন্য বিশেষভাবে তৈরি সম্পূর্ণ সমাধান। মেয়াদ ট্র্যাক করুন, আয় ম্যানেজ করুন, এক ড্যাশবোর্ড থেকে সব নিয়ন্ত্রণ করুন।');
  const features = getCMSValue(cmsContent, 'items', fallbackFeatures);
  const highlights = getCMSValue(cmsContent, 'highlights', fallbackHighlights);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((feature: any, index: number) => {
            ...
            return (
              ...
              >
                {/* Gradient Overlay */}
                <div className={`absolute inset-0 bg-gradient-to-br ${feature.gradient || 'from-primary/20 to-primary/10'} opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl`} />
