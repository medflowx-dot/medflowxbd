import { UserPlus, Settings, Rocket, Package, TrendingUp, LucideIcon } from 'lucide-react';
...
const HowItWorks = () => {
  const { data: cmsContent } = useCMSContent('how_it_works');
  
  const title = getCMSValue(cmsContent, 'title', 'মাত্র ৩টি ধাপে শুরু করুন');
  const subtitle = getCMSValue(cmsContent, 'subtitle', 'সাইনআপ থেকে পূর্ণ কার্যক্ষম — মাত্র কয়েক মিনিটে।');
  const steps = getCMSValue(cmsContent, 'steps', fallbackSteps);
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4 relative">
            ...
            {steps.map((step: any, index: number) => {
              ...
              const stepNumber = step.number || `০${index + 1}`;
